...
- 소개: 프롭테크 기업
- 기간: 21.04 ~ 현재
- 사용기술: `React`, `Typescript`, Redux, redux-toolkit, styped-components, reat-query,`Jest`
- 작업 범위:
- 상세보기: [🔍보기]()
- 작업 내용
...
- 기술을 좋아하지만, 생산성과 효용성을 고려하며 개발자의 아집을 버리기 위해 노력합니다.
...
### 1.프론트엔드는 어떤 역할이라 생각하는지?
    ...
    물 흐르듯 자연스러운 경험을 제공하며 이탈을 최소화는 것
...
### 5. 코드리뷰시 중요하게 생각하는 가치는?

1. 상대방의 의도를 이해하고 존중하는 것.
2. 상대가 선한 마음으로 더 나은 결과를 위해 리뷰를 했다는 믿음을 갖는 것.
3. 내 생각이 틀렸을 수 있다는 여지를 남기는 것.
4. 구체적 개선 포인트, 리스크 요인을 설명하는 것.

   "이 props이 이 컴포넌트에서 어떤 역할을 수행하는지 애매하네요. 이렇게 이름을 바꿔보면 어떨까요?"  
   "의존성 배열에 이 요소 때문에 effect함수가 불필요하게 실행될 것 같은데 이렇게 바꿔보면 어떨까요?"

### 6. 개발에서 중요하게 여기는 가치가 있다면?
...
그 다음으로 중요한 것이 `유지보수`입니다. `유지보수`하기 쉬우려면 코드가 모듈화되어 `가독성`이 좋아야 시장에 빠르게 대응할 수 있습니다. `유지보수가 쉽다`는 것은 새로운 요구사항에 빠르고 유연하게 대처할 수 있다는 것을 의미합니다. 다시말해 변화에 적응해서 계속 **가독성이 좋은 코드가 시장에서 생존할 수 있는 코드**라 생각합니다.

### 7. 2022년 어떤 생각을 하고 있는지?
...
### 8. 팀 플레이에서 중요하게 생각하는 것이 있다면?

- 말을 독점하지 않는것.
- 팀의 기술적 공감대를 존중하는것.
- 팀원의 페인 포인트에 공감하는것.
- 팀원의 시간을 아껴줄 방법을 고민하는 것.
